...
const BestSelling = () => {
    ...
    // Filter only the best-selling products
    const bestSellingProducts = featuresProduct.filter((item) => item.bestSelling);
    const displayedProducts = showAll ? bestSellingProducts : bestSellingProducts.slice(0, 4);
                    ...
                    <div>
                        {showAll ? (
                            <div className="flex justify-center">
                                <CmnButton onClick={handleHideAll} title={'Hide All'} />
                            </div>
                        ) : (
                            <div className="flex justify-center">
                                <CmnButton onClick={handleViewAll} title={'View All'} />
                            </div>
                        )}
                    </div>
                </div>
                <div className="flex flex-wrap justify-between gap-y-8 gap-x-4 mb-10 md:mb-[50px] cursor-pointer">
                    {displayedProducts.map((item, index) => (
                        <div
                            key={index}
                            className="w-full sm:w-[48%] md:w-[32%] lg:w-[270px] h-auto relative"
                        >
                            ...
                            <div className="mt-4">
                                ...
                                <div className="flex gap-3 mb-2">
                                    ...
                                    <del className="text-sm md:text-base font-medium text-[#7D8184]">
                                        {item.price}
                                    </del>
                                </div>
                                <div className="flex gap-2">
                                    ...
                                    <p className="text-[#7D8184] text-xs md:text-sm font-semibold">
                                        {item.totalReviews}
                                    </p>
                                </div>
                            </div>
